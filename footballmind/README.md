...
> **Repo layout note:** the FootballMind app lives in this `footballmind/` folder.
> Repo-level automation stays one directory up: `../render.yaml` points Render here,
> and `../.github/workflows/` runs jobs with `working-directory: footballmind`.

From the repository root:

```
├── README.md                  # Repo-level pointer to this app folder
├── render.yaml                # Render deploy config (builds/runs footballmind/)
├── .github/workflows/         # CI, scheduled jobs, and Pages deploy
└── footballmind/
    ...
    ├── footballmind_services.py
    ├── footballmind_sync.py   # football-data.org ingestion
    ├── footballmind_jobs.py   # CLI: sync, sync-matchday, quick-refit, retrain, ...
    ├── footballmind_enrich.py # FPL + API-Football + Understat enrichment
    ├── footballmind_sofifa.py # EA FC / SoFIFA attributes (optional sync)
    ├── footballmind_wikipedia.py
    ├── footballmind_elo.py
